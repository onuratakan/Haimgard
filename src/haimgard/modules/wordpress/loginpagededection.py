...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "wordpress/loginpagededection"
        self.description = "WordPress login page detection."
    ...
    def run(self):
        ...
        url2 = f"{url}/wp-login.php"
        r = requests.get(url2, headers={"User-Agent":random.choice(user_agents)}, verify = sslverify)
        print(f"\033[32m[+]\033[0m WordPress login page is detected on {url2}") if r.status_code == 200 else print(f"[-] WordPress login page is not detected on {url2}")

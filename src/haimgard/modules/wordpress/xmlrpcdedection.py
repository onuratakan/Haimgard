...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "wordpress/xmlrpcdedection"
        self.description = "WordPress xmlrpc detection."
    ...
    def run(self):
        ...
        url2 = f"{url}/xmlrpc.php"
        r = requests.get(url2, headers={"User-Agent":random.choice(user_agents)}, verify = sslverify)
        print(f"\033[32m[+]\033[0m WordPress xmlrpc is detected on {url2}") if r.status_code == 200 or r.status_code == 405 else print(f"[-] WordPress xmlrpc is not detected on {url2}")

...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "wordpress/readmededection"
        self.description = "WordPress readme detection."
    ...
    def run(self):
        ...
        url2 = f"{url}/readme.html"
        r = requests.get(url2, headers={"User-Agent":random.choice(user_agents)}, verify = sslverify)
        print(f"\033[32m[+]\033[0m WordPress readme is detected on {url2}") if r.status_code == 200 else print(f"[-] WordPress readme is not detected on {url2}")

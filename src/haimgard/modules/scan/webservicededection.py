...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "scan/webservicededection"
        self.description = "WEB service dedection."
    ...
    def run(self):
        ...
        wwebservice_found = False
        for service in services:
            if service[0] in r.text:
                wwebservice_found = True
                table.add_row(service[1])


        if wwebservice_found:
            print(f"\033[32m[+]\033[0m WEB service is detected on {url}")
            console.print(table)
        else:
            print(f"[-] WEB service is not detected on {url}")

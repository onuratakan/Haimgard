...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "scan/scriptbroadcast"
        self.description = "Script broadcast dedection."
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=broadcast')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                broadcast += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), broadcast)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script broadcast on {target}")
            console.print(table)
        else:
            print(f"[-] Any script broadcast founded on {target}")
...
class Module:
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=external')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                external += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), external)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script external on {target}")
            console.print(table)
        else:
            print(f"[-] Any script external founded on {target}")
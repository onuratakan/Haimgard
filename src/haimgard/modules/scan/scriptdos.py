...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "scan/scriptdos"
        self.description = "Script dos dedection."
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=dos')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                dos += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), dos)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script dos on {target}")
            console.print(table)
        else:
            print(f"[-] Any script dos founded on {target}")
...
class Module:
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=discovery')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                discovery += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), discovery)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script discovery on {target}")
            console.print(table)
        else:
            print(f"[-] Any script discovery founded on {target}")
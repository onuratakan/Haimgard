...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "scan/scriptbrute"
        self.description = "Script brute dedection."
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=brute')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                brute += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), brute)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script brute on {target}")
            console.print(table)
        else:
            print(f"[-] Any script brute founded on {target}")
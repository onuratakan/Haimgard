...
class Module:
    ...
    def run(self):
        ...
        url2 = f"{url}/wp-content/uploads/"
        r = requests.get(url2, headers={"User-Agent":random.choice(user_agents)}, verify = sslverify)
        print(f"\033[32m[+]\033[0m WordPress uploads is detected on {url2}") if r.status_code == 200 else print(f"[-] WordPress uploads is not detected on {url2}")

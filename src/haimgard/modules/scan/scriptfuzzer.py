...
class Module:
    def __init__(self, logger):
        self.logger = logger
        self.name = "scan/scriptfuzzer"
        self.description = "Script fuzzer dedection."
    ...
    def run(self):
        ...
        nm = nmap.PortScanner()
        nm.scan(target, arguments='--script=fuzzer')
                ...
                for port in lport:
                        if nm[host][proto][port].get("script") is not None:
                            ...
                            for key, value in nm[host][proto][port]["script"].items():
                                fuzzer += f"{key} : {value}\n"
                            open_service = True
                            table.add_row(str(nm[host][proto][port]["name"]) + "/" + str(proto) + "/" + str(port), fuzzer)

        if open_service:
            print(f"\033[32m[+]\033[0m Founded some script fuzzer on {target}")
            console.print(table)
        else:
            print(f"[-] Any script fuzzer founded on {target}")